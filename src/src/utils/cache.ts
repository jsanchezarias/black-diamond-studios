...
export const CacheSystem = {
    ...
    set: <T>(key: string, data: T, ttlInMinutes: number = 60): void => {
        try {
            ...
            console.log(`💾 Guardado en caché local: ${key}`);
        } catch (error) {
            console.warn(`⚠️ Error al guardar en caché (${key}):`, error);
        }
    },
    ...
    get: <T>(key: string): T | null => {
        try {
            ...
            console.log(`⚡ Recuperado del caché instantáneo: ${key}`);
            return entry.data;
        } catch (error) {
            console.warn(`⚠️ Error al leer caché (${key}):`, error);
            return null;
        }
    },
