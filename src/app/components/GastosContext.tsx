...
export function GastosProvider({ children }: { children: ReactNode }) {
      ...
      if (serviciosParaNotificar.length > 0) {
        ...
        for (const servicio of serviciosParaNotificar) {
          try {
            ...
            if (process.env.NODE_ENV === 'development') console.error('Error actualizando notificación:', error);
          }
        }
  ...
  const editarGasto = async (id: string, gastoActualizado: Partial<GastoOperativo>) => {
    try {
      ...
      if (process.env.NODE_ENV === 'development') console.error('❌ Error editando gasto:', error);
      throw error;
    }
  };
  ...
  const eliminarGasto = async (id: string) => {
    try {
      ...
      if (process.env.NODE_ENV === 'development') console.error('❌ Error eliminando gasto:', error);
      throw error;
    }
  };
  ...
  const agregarServicio = async (servicio: Omit<ServicioPublico, 'id' | 'notificacionEnviada'>) => {
    try {
      ...
      if (process.env.NODE_ENV === 'development') console.error('❌ Error agregando servicio:', error);
      throw error;
    }
  };
  ...
  const editarServicio = async (id: string, servicioActualizado: Partial<ServicioPublico>) => {
    try {
      ...
      if (process.env.NODE_ENV === 'development') console.error('❌ Error editando servicio:', error);
      throw error;
    }
  };
  ...
  const eliminarServicio = async (id: string) => {
    try {
      ...
      if (process.env.NODE_ENV === 'development') console.error('❌ Error eliminando servicio:', error);
      throw error;
    }
  };
