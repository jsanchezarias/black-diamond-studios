...
export function AnalyticsProvider({ children }: AnalyticsProviderProps) {
  ...
  const { gastosOperativos: gastos } = useGastos();
  const { multas } = useMultas();
  
  const [ventasBoutique, setVentasBoutique] = useState<any[]>([]);
  ...
  const obtenerMetricasGenerales = useCallback((filtros?: FiltrosAnalytics): MetricasGenerales => {
    ...
    // Ingresos por período (Servicios)
    ...
    // Ingresos Boutique
    const ventasBoutiqueMes = (ventasBoutique || []).filter(v => {
      const f = v.fecha ? v.fecha.split('T')[0] : '';
      return estaEnRango(f, inicioMes, hoy);
    });
    const ingresosBoutiqueMes = (ventasBoutiqueMes || []).reduce((sum, v) => sum + (Number(v.total) || 0), 0);
    ...
    const multasMes = (multas || []).filter(m => {
      const fechaMulta = (m.fecha || '').split('T')[0];
    ...
    const gastosMes = (gastos || []).filter(g => {
      const fechaGasto = g.fecha ? (typeof g.fecha === 'string' ? g.fecha : g.fecha.toISOString()).split('T')[0] : '';
      return estaEnRango(fechaGasto, inicioMes, hoy);
    });
    const gastosOperativosMes = (gastosMes || []).reduce((sum, g) => sum + (g.monto || 0), 0);

    // Utilidad neta y margen (Incluyendo Boutique)
    const ingresosTotalesMes = ingresosMes + ingresosBoutiqueMes;
    const utilidadNeta = ingresosTotalesMes - gastosOperativosMes;
    const margenUtilidad = ingresosTotalesMes > 0 ? (utilidadNeta / ingresosTotalesMes) * 100 : 0;
  ...
  }, [servicios, clientes, modelos, multas, gastos, ventasBoutique]);
  ...
  const obtenerReporteFinanciero = useCallback((filtros?: FiltrosAnalytics): ReporteFinanciero => {
    ...
    const multasPeriodo = (multas || []).filter(m => {
      const fechaMulta = (m.fecha || '').split('T')[0];
      return estaEnRango(fechaMulta, fechaInicio, fechaFin) && m.estado === 'pagada';
    });
    const ingresosPorMultas = (multasPeriodo || []).reduce((sum, m) => sum + (m.monto || 0), 0);
    
    // Ingresos Boutique
    const ventasPeriodo = (ventasBoutique || []).filter(v => {
      const f = v.fecha ? v.fecha.split('T')[0] : '';
      return estaEnRango(f, fechaInicio, fechaFin);
    });
    const ingresosOtros = (ventasPeriodo || []).reduce((sum, v) => sum + (Number(v.total) || 0), 0);
    ...
    const gastosTotal = (gastosPeriodo || []).reduce((sum, g) => sum + (g.monto || 0), 0);
  ...
  }, [aplicarFiltrosServicios, multas, gastos, ventasBoutique]);
    ...
    if (tipo === 'ingresos') {
      ...
      (multas || [])
        ...
        .forEach(m => {
          ...
          datosPorFecha.set(fecha, valor + (m.monto || 0));
        });
    }
