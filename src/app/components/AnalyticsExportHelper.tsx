...
export function exportarCSV(datos: any[], nombreArchivo: string) {
  try {
    if (!datos || datos.length === 0) {
      if (process.env.NODE_ENV === 'development') console.warn('⚠️ No hay datos para exportar');
      return;
    }
    ...
    if (process.env.NODE_ENV === 'development') console.error('❌ Error exportando CSV:', error);
  }
}
...
export function exportarJSON(datos: any, nombreArchivo: string) {
  try {
    ...
    if (process.env.NODE_ENV === 'development') console.error('❌ Error exportando JSON:', error);
  }
}
...
export function exportarReporteHTML(
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') console.error('❌ Error exportando reporte HTML:', error);
  }
}
...
export function imprimirReporte(
  ...
  try {
    ...
    if (ventanaImpresion) {
      ...
      if (process.env.NODE_ENV === 'development') console.error('❌ No se pudo abrir la ventana de impresión');
    }
  } catch (error) {
    if (process.env.NODE_ENV === 'development') console.error('❌ Error imprimiendo reporte:', error);
  }
}
