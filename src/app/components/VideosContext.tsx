...
export function VideosProvider({ children }: { children: ReactNode }) {
  ...
  const cargarVideos = async () => {
    try {
      ...
      if (process.env.NODE_ENV === 'development') console.error('Error al cargar videos:', error);
      setVideos([]);
  ...
  const agregarVideo = async (videoData: Omit<Video, 'id' | 'fechaCreacion' | 'fechaActualizacion'>): Promise<Video> => {
    try {
      ...
      if (process.env.NODE_ENV === 'development') console.error('Error al agregar video:', error);
  ...
  const actualizarVideo = async (id: string, data: Partial<Video>) => {
    try {
      ...
      if (process.env.NODE_ENV === 'development') console.error('Error al actualizar video:', error);
  ...
  const eliminarVideo = async (id: string) => {
    try {
      ...
      if (video.urlStorage) {
        ...
        if (storageError) {
          if (process.env.NODE_ENV === 'development') console.error('Error al eliminar archivo de storage:', storageError);
        }
      }
      ...
      if (process.env.NODE_ENV === 'development') console.error('Error al eliminar video:', error);
  ...
  const subirArchivoVideo = async (
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') console.error('Error al subir video:', error);
  ...
  const reordenarVideos = async (videosReordenados: Video[]) => {
    try {
      ...
      if (process.env.NODE_ENV === 'development') console.error('Error al reordenar videos:', error);
