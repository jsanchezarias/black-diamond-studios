  ...
  const handleLogout = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (logoutRef?.current) {
        ...
        if (process.env.NODE_ENV === 'development') console.error('🔴 logoutRef.current no está disponible');
      }
    } catch (error) {
      if (process.env.NODE_ENV === 'development') console.error('🔴 Error al cerrar sesión desde LiveChat:', error);
    }
  };
