...
export function ServiciosProvider({ children }: { children: ReactNode }) {
  ...
  const cargarServicios = async () => {
    try {
      ...
      if (process.env.NODE_ENV === 'development') console.error('❌ Error inesperado cargando servicios:', error);
      setServicios([]);
    } finally {
      setLoading(false);
    }
  ...
  const crearServicio = async (servicio: Omit<Servicio, 'id' | 'fechaCreacion' | 'creadoPor'>) => {
    try {
      ...
      if (error) {
        if (process.env.NODE_ENV === 'development') console.error('❌ Error creando servicio:', error.message);
      ...
      if (process.env.NODE_ENV === 'development') console.error('❌ Error en crearServicio:', error);
      return { success: false, error };
    }
  };
  ...
  const actualizarServicio = async (id: string, servicio: Partial<Servicio>) => {
    try {
      ...
      if (process.env.NODE_ENV === 'development') console.error('❌ Error en actualizarServicio:', error);
      throw error;
    }
  };
