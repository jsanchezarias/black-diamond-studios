...
export function PagosProvider({ children }: { children: ReactNode }) {
  ...
  const aprobarAdelanto = (adelantoId: string, aprobadoPor: string) => {
    ...
    if (adelanto) {
      ...
      }).catch(err => { if (process.env.NODE_ENV === 'development') console.error('Error notificando adelanto aprobado:', err); });
    }
  };
  ...
  const rechazarAdelanto = (adelantoId: string, aprobadoPor: string) => {
    ...
    if (adelanto) {
      ...
      }).catch(err => { if (process.env.NODE_ENV === 'development') console.error('Error notificando adelanto rechazado:', err); });
    }
  };
