...
export function ModeloDashboard({ accessToken, userId, userEmail, onLogout }: ModeloDashboardProps) {
  ...
  // Derivar arrays que antes se esperaban del contexto pero no existen
  const serviciosFinalizados = servicios.filter(s => s.estado === 'completado');
  const serviciosActivos = servicios.filter(s => s.estadoPago === 'pendiente');
  const obtenerServicioActivo = (email: string) =>
    servicios.find(s => s.modeloEmail === email && s.estadoPago === 'pendiente');
  ...
  const puedeIniciarServicio = !!registroActivo; // Ahora depende de si tiene un turno activo real
  ...
  const multasPendientes = obtenerTotalMultasPendientesPorEmail(modeloActual?.email || '');
  const adelantosPendientes = obtenerAdelantosPendientes().filter(a => a.modeloEmail === modeloActual?.email).length;
  ...
  return (
    <div className="min-h-screen w-full bg-background" style={{ fontFamily: 'Montserrat, sans-serif' }}>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-card/98 backdrop-blur-premium border-b border-primary/15 shadow-premium">
          ...
          <div className="flex items-center gap-3 min-w-0 flex-1">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="text-[10px] h-4 px-1 border-primary/30 text-primary/80">
        ...
        {menuOpen && (
          <div className="bg-card/95 backdrop-blur-md border-t border-primary/10 shadow-lg md:hidden">
            <nav className="flex flex-col px-4 py-3 space-y-2 max-w-7xl mx-auto">
              <div className="grid grid-cols-2 gap-2 mb-4">
                <Button 
                  onClick={() => { setSelectedTab('resumen'); setMenuOpen(false); }}
                  variant={selectedTab === 'resumen' ? 'default' : 'ghost'}
                  className="justify-start h-10 text-xs"
                >
                  <Activity className="w-4 h-4 mr-2" /> Resumen
                </Button>
                <Button 
                  onClick={() => { setSelectedTab('servicios'); setMenuOpen(false); }}
                  variant={selectedTab === 'servicios' ? 'default' : 'ghost'}
                  className="justify-start h-10 text-xs"
                >
                  <CheckCircle className="w-4 h-4 mr-2" /> Servicios
                </Button>
                <Button 
                  onClick={() => { setSelectedTab('ingresos'); setMenuOpen(false); }}
                  variant={selectedTab === 'ingresos' ? 'default' : 'ghost'}
                  className="justify-start h-10 text-xs"
                >
                  <DollarSign className="w-4 h-4 mr-2" /> Ingresos
                </Button>
                <Button 
                  onClick={() => { setSelectedTab('transmision'); setMenuOpen(false); }}
                  variant={selectedTab === 'transmision' ? 'default' : 'ghost'}
                  className="justify-start h-10 text-xs"
                >
                  <Video className="w-4 h-4 mr-2" /> Transmisión
                </Button>
              </div>
              {onLogout && (
                <>
