...
export function StreamingPage() {
  ...
  const fetchActiveSessions = async () => {
    try {
      ...
      if (!error && data) {
        setActiveSessions(data.map((row: any) => ({
          id: row.id,
          modelId: row.sede_id || row.id,
          modelName: row.nombre || row.sede_id || 'En Vivo',
      ...
      if (process.env.NODE_ENV === 'development') console.error('Error fetching sessions:', error);
    } finally {
      setLoading(false);
    }
  };
          ...
          <Card className="border-primary/20 bg-gradient-card">
            <CardContent className="p-12 text-center">
              ...
              <div className="flex items-center justify-center gap-2 text-primary/60">
                ...
                <span className="text-sm font-medium">Mantente al tanto para nuevas transmisiones</span>
              </div>
            </CardContent>
          </Card>
        )}
