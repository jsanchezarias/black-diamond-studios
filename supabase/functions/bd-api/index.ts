...
const FUNCTION_BASE_PATH = '/bd-api';
const app = new Hono().basePath(FUNCTION_BASE_PATH);
...
app.use('*', async (c, next) => {
  ...
  const isBaseRequest = c.req.path === '/' || c.req.path === '' || c.req.path === FUNCTION_BASE_PATH || c.req.path === `${FUNCTION_BASE_PATH}/`;
  if (invokePath && isBaseRequest) {
    console.log(`🔀 Re-enrutando petición '${c.req.path}' -> '${invokePath}' vía x-invoke-path`);
    const url = new URL(c.req.url);
    url.pathname = FUNCTION_BASE_PATH + invokePath;
