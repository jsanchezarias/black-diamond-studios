...
// Cliente con permisos completos (service role) — inicialización perezosa:
// crearlo a nivel de módulo puede tumbar el arranque de toda la función si
// las env vars no están disponibles en ese instante.
...
export async function ensureBucketExists() {
  ...
  if (!bucketExists) {
    ...
    const { data, error: createError } = await getSupabase().storage.createBucket(
      BUCKET_NAME,
      {
        public: true,
...
export async function deleteFile(fileName: string): Promise<void> {
  const { error } = await getSupabase().storage.from(BUCKET_NAME).remove([fileName]);
